...
print(f"[STARTUP] ASSEMBLYAI_API_KEY loaded: {'Yes' if os.getenv('ASSEMBLYAI_API_KEY') else 'No'}")
print(f"[STARTUP] CARTESIA_API_KEY loaded: {'Yes' if os.getenv('CARTESIA_API_KEY') else 'No'}")
print(f"[STARTUP] ANTHROPIC_API_KEY loaded: {'Yes' if os.getenv('ANTHROPIC_API_KEY') else 'No'}")
...
agent = create_agent(
    model="bedrock:anthropic.claude-3-5-haiku-20241022-v1:0",  # Using Claude 3.5 Haiku on AWS Bedrock
...
async def _agent_stream(
    ...
    async for event in event_stream:
        print(f"[DEBUG AGENT] Received event type: {event.type}")
        ...
        if event.type == "stt_output":
            print(f"[DEBUG AGENT] Invoking agent with transcript: {event.transcript}")
