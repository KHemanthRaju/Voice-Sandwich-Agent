...
class AssemblyAISTT:
                    ...
                    async for raw_message in self._ws:
                        try:
                            ...
                            if message_type == "Begin":
                                print("[DEBUG] Begin message received")
                                ...
                                print(f"[DEBUG] Turn event - transcript: '{transcript}', formatted: {turn_is_formatted}")

                                if turn_is_formatted:
                                    if transcript:
                                        print(f"[DEBUG] Emitting STTOutputEvent: {transcript}")
    ...
    async def send_audio(self, audio_chunk: bytes) -> None:
        ...
        print(f"[DEBUG] Sending audio chunk: {len(audio_chunk)} bytes")
    ...
    async def close(self) -> None:
        if self._ws and self._ws.close_code is None:
            try:
                ...
                print("[DEBUG] Sending terminate message to force turn formatting")
                await self._ws.send(json.dumps({"terminate_session": True}))
                # Give it a moment to process
                await asyncio.sleep(0.1)
            except Exception as e:
                print(f"[DEBUG] Error sending terminate: {e}")
    ...
    async def _ensure_connection(self) -> WebSocketClientProtocol:
        ...
        print(f"[DEBUG] Connecting to AssemblyAI: {url}")
        self._ws = await websockets.connect(
            url, additional_headers={"Authorization": self.api_key}
        )
        print("[DEBUG] Connected to AssemblyAI WebSocket")
