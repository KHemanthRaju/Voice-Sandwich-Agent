...
config({ path: envPath });
console.log(`[STARTUP] ASSEMBLYAI_API_KEY loaded: ${process.env.ASSEMBLYAI_API_KEY ? 'Yes' : 'No'}`);
console.log(`[STARTUP] CARTESIA_API_KEY loaded: ${process.env.CARTESIA_API_KEY ? 'Yes' : 'No'}`);
console.log(`[STARTUP] ANTHROPIC_API_KEY loaded: ${process.env.ANTHROPIC_API_KEY ? 'Yes' : 'No'}`);
...
const agent = createAgent({
  model: bedrockModel,  // Use explicit Bedrock model instance
...
async function* agentStream(
  ...
  for await (const event of eventStream) {
    console.log(`[DEBUG AGENT] Received event type: ${event.type}`);
    yield event;
    if (event.type === "stt_output") {
      console.log(`[DEBUG AGENT] Invoking agent with transcript: ${event.transcript}`);
      try {
        ...
        console.log(`[DEBUG AGENT] Agent stream started, waiting for messages...`);
        for await (const [message] of stream) {
          console.log(`[DEBUG AGENT] Received message from agent:`, message);
      ...
      yield { type: "agent_end", ts: Date.now() };
        ...
        console.error(`[DEBUG AGENT] Error invoking agent:`, error);
      }
    }
  }
}
